...
try {
    // 1. Get the specific credit to pay
    $getCreditQuery = "SELECT debt_id, due_amount FROM debt_payments 
                    WHERE debt_id = ? AND company_id = ? AND name = ? 
                    AND debtor_creditor = 'creditor' AND due_amount > 0";
    $stmt = $conn->prepare($getCreditQuery);
    $stmt->bind_param("iis", $debt_id, $company_id, $creditorName);
    $stmt->execute();
    $credit = $stmt->get_result()->fetch_assoc();
    
    if (!$credit) {
        throw new Exception("Credit not found or already paid");
    }
    
    $amountToPay = min($credit['due_amount'], $paymentAmount);
    
    // 2. Update the credit record
    $updateCreditQuery = "UPDATE debt_payments SET due_amount = due_amount - ? 
                       WHERE debt_id = ?";
    ...
    foreach ($paymentMethods as $payment) {
        ...
        // Update money balance for this method (subtract since we're paying a creditor)
